...
export default function Sidebar({
  ...
  return (
    <aside className="flex flex-col w-64 min-h-screen bg-[#0c0f1a] border-r border-white/[0.06]">
      {/* Logo */}
      <div className="flex items-center px-5 h-16 border-b border-white/[0.06] shrink-0">
        <Image src="/image.png" alt="Merge" width={100} height={52} className="object-contain brightness-0 invert" />
      </div>
      ...
      <div className="px-3 pb-4 border-t border-white/[0.06] pt-3 space-y-1">
        {bottomLinks.map((link) => (
          <button
            key={link.id}
            className="group flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-sm font-medium text-white/40 hover:bg-white/[0.05] hover:text-white/70 transition-all duration-150"
          >
        ...
        <div className="flex items-center gap-3 px-3 py-2.5 mt-1 rounded-xl hover:bg-white/[0.05] cursor-pointer transition-all duration-150">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-white/70 truncate">My Workspace</p>
          </div>
        </div>
      </div>
    </aside>
